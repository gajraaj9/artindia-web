...
const ENV = { BREVO_API_KEY: 'test-key', WA_VERIFY_TOKEN: 'sekrit' };
...
test('arrêt works too, and an ordinary message is left alone', async () => {
  ...
  await post(ENV, change({
    messages: [{ from: '32490616661', text: { body: 'where do I park?' } }],
  }));
  assert.equal(db.get('anouk@example.com').attributes.WA_OPTIN, true);
  assert.equal(calls.length, 0, 'a question is not an opt-out');
});
